---
title: "Claude Code 서브에이전트로 블로그 자동화 파이프라인 만들기: 리서치부터 발행까지"
...
역할을 나눴다면, 다음은 각 역할에 맞는 모델을 고르는 차례예요. 라우팅이나 형식 정리처럼 단순한 작업은 Haiku, 본작업 대부분은 Sonnet, 어려운 추론이 필요한 10~15%만 Opus에 맡기는 방식입니다. 이 하이브리드 전략을 적용하면 "60~70%의 비용 절감(60-70% cost reduction)" 효과가 있다는 보고도 있어요.
